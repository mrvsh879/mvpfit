...
const exerciseBank = {
  warmup: [
    { type: "Разминка", name: "Дорожка лёгкая", details: "6–8 минут, пульс спокойно повышается", note: "Не стартуй резко. Цель — разогреть тело." },
    { type: "Разминка", name: "Суставная разминка", details: "Шея, плечи, локти, кисти, таз, колени, голеностоп — 5 минут", note: "Делай плавно, без боли." },
    { type: "Разминка", name: "Резинка + активация", details: "Тяги резинки, разведения, 2×15", note: "Особенно важно перед жимом и подтягиваниями." }
  ],
  stretch: [
    { type: "Растяжка", name: "Грудь и плечи", details: "2–3 минуты", note: "После жима и мешка не зажимай плечи." },
    { type: "Растяжка", name: "Бёдра и задняя поверхность", details: "4–5 минут", note: "Дыши спокойно. Не рви амплитуду." },
    { type: "Растяжка", name: "Спина + мобилити", details: "Кошка-корова, наклоны, раскрытие грудного отдела", note: "Цель — восстановление, а не боль." }
  ]
};
...
function healthRisk(profile) {
  const text = (profile.health || "").toLowerCase();
  const risky = ["серд", "давлен", "гипер", "колен", "спин", "грыж", "травм", "операц", "боль"];
  return risky.some(word => text.includes(word));
}

function workingWeight(maxOrKnown, fallback, intensity) {
  const base = maxOrKnown > 0 ? maxOrKnown : fallback;
...
function buildWorkout(profile, week, template) {
  ...
  const intensityBase = level === "advanced" ? 0.72 : level === "intermediate" ? 0.62 : 0.50;
  const weekAdd = (week - 1) * 0.035;
  const intensity = risk ? Math.min(0.50, intensityBase) : intensityBase + weekAdd;
  const reps = repsByGoal(profile.goal, week);

  const benchFallback = profile.weight * 0.55;
  const squatFallback = profile.weight * 0.65;
  const dumbbell = level === "beginner" ? 8 + week : level === "intermediate" ? 12 + week : 16 + week;

  const warm = exerciseBank.warmup.map(x => ({...x}));
  const stretch = exerciseBank.stretch.map(x => ({...x}));
  ...
  if (template === "push") {
    main = [
      makeExercise("Сила", "Жим лёжа", workingWeight(profile.bench, benchFallback, intensity), 4, reps, "Перед рабочим весом сделай 2 лёгких разминочных подхода."),
      makeExercise("Сила", "Жим гантелей на скамье", dumbbell, 3, "8–12", "Контролируй опускание 2 секунды."),
      makeExercise("Плечи", "Жим стоя с грифом", workingWeight(profile.bench, 30, intensity * .55), 3, "8–10", "Не прогибай поясницу."),
      makeExercise("Плечи", "Махи гантелями в стороны", Math.max(3, Math.round(dumbbell * .45)), 3, "12–15", "Локти слегка согнуты."),
      makeExercise("Трицепс", "Французский жим", workingWeight(profile.bench, 22, intensity * .45), 3, "10–12", "Локти не разводить.")
    ];
    finisher = [
      { type: "Груша", name: "Боксёрский мешок", details: `${4 + week} раундов × 2 минуты`, note: "Работай технично. Не бей на максимум, если плечи устали." }
    ];
  }

  if (template === "pull") {
    main = [
      makeExercise("Спина", "Подтягивания", null, 4, profile.pullups >= 5 ? "макс -1" : "с резинкой 6–8", "Остановись за 1 повтор до отказа."),
      makeExercise("Спина", "Тяга штанги к поясу", workingWeight(profile.bench || profile.squat, profile.weight * .45, intensity), 4, "8–12", "Спина ровная, тяни локтями."),
      makeExercise("Спина", "Тяга гантели одной рукой", dumbbell + 2, 3, "10–12", "Пауза в верхней точке."),
      makeExercise("Трапеции", "Шраги с гантелями", dumbbell + 4, 3, "12–15", "Без рывков."),
      makeExercise("Бицепс", "Подъём грифа на бицепс", workingWeight(profile.bench, 20, intensity * .45), 3, "10–12", "Не раскачивай корпус."),
      makeExercise("Бицепс", "Молотки", dumbbell, 3, "10–12", "Медленно опускай.")
    ];
  }

  if (template === "legs") {
    main = [
      makeExercise("Ноги", "Присед со штангой", workingWeight(profile.squat, squatFallback, intensity), 4, reps, risk ? "Если колени/спина беспокоят — снизь вес и амплитуду." : "Колени по линии носков."),
      makeExercise("Ноги", "Румынская тяга", workingWeight(profile.squat, profile.weight * .55, intensity), 4, "8–12", "Таз назад, спина нейтрально."),
      makeExercise("Ноги", "Выпады с гантелями", dumbbell, 3, "10 на ногу", "Держи корпус ровно."),
      makeExercise("Ноги", "Болгарские приседы", Math.max(4, dumbbell - 2), 3, "8–10 на ногу", "Если тяжело — без веса."),
      makeExercise("Икры", "Подъёмы на икры", dumbbell + 4, 4, "15–20", "Пауза сверху."),
      makeExercise("Кор", "Планка", null, 3, "30–60 сек", "Не провисай в пояснице.")
    ];
  }

  if (template === "functional") {
    main = [
      { type: "Круг", name: "Круговая тренировка", details: `${3 + Math.floor(week / 2)} круга: отжимания 12, тяга резинки 15, присед 15, пресс 20`, note: "Отдых между кругами 90 секунд." },
      makeExercise("Турник", "Подтягивания / вис", null, 3, profile.pullups >= 3 ? "макс -1" : "вис 20–30 сек", "Качаем спину и хват."),
      makeExercise("Гантели", "Жим гантелей стоя", dumbbell, 3, "10–12", "Без рывков."),
      { type: "Груша", name: "Мешок интервальный", details: `${5 + week} раундов: 40 сек работа / 20 сек отдых`, note: "Техника выше силы удара." },
      { type: "Кардио", name: "Дорожка", details: cardioByGoal(profile.goal, week), note: "Темп разговорный или интервалы 1/1, если цель выносливость." }
    ];
  }

  if (template === "recovery") {
    main = [
      { type: "Кардио", name: "Дорожка восстановительная", details: cardioByGoal(profile.goal, week), note: "Пульс умеренный. После тренировки должно стать легче, не хуже." },
      { type: "Мобилити", name: "Мобилити таза и грудного отдела", details: "12–15 минут", note: "Медленно, с дыханием." },
      { type: "Резинка", name: "Плечи и лопатки с резинкой", details: "3×15", note: "Профилактика плеч после жима и мешка." },
      { type: "Груша", name: "Лёгкий мешок", details: "3 раунда × 2 минуты", note: "Только техника, без рубки." }
    ];
  }
...
function generatePlan(profile) {
  ...
  for (let week = 1; week <= 4; week++) {
    const weekDays = templates.map((tpl, dayIndex) => {
      if (tpl === "rest") {
        return {
      ...
      const exercises = buildWorkout(profile, week, tpl);
      return {
        id: `w${week}-d${dayIndex + 1}`,
        week,
        dayIndex,
...
function renderPlan() {
  ...
  container.innerHTML = state.plan.map(week => `
    <div class="week-block">
      <div class="week-head">
        ...
        <span>${week.days.filter(d => d.exercises.length).length} тренировочных дней</span>
      </div>
      ${week.days.map(day => `
        <div class="day-card">
          <div class="day-meta">
          ...
          <div>
            ...
            <div class="badges">
              ${day.exercises.length ? `<span class="badge">${day.exercises.length} шагов</span>` : `<span class="badge rest">Отдых</span>`}
          ...
          <div>
            ${day.exercises.length ? `<button class="primary-btn" data-start="${day.id}">${day.completed ? "Повторить" : "Начать"}</button>` : `<button class="ghost-btn" data-rest="${day.id}">Отметить отдых</button>`}
          </div>
        </div>
      `).join("")}
    </div>
  ...
  container.querySelectorAll("[data-rest]").forEach(btn => {
    btn.addEventListener("click", () => markRest(btn.dataset.rest));
  });
}
...
function renderWorkout() {
  ...
  $("liveExerciseList").innerHTML = day.exercises.map((ex, index) => {
    const result = active.results[index];
    const statusText = result.status === "done" ? `✓ ${formatTime(result.durationMs)}` :
      result.status === "skipped" ? `Пропущено` :
      index === active.currentIndex ? "Сейчас" : "Ожидает";
    ...
    return `
      <div class="live-exercise-row ${index === active.currentIndex ? "active" : ""} ${result.status === "done" ? "done" : ""} ${result.status === "skipped" ? "skipped" : ""}">
        ...
        <div>
          <b>${ex.name}</b>
          <div class="row-sub">${ex.type} · ${ex.details}</div>
        </div>
...
function completeCurrentExercise(status) {
  ...
  if (index >= day.exercises.length) return;
...
function finishWorkout() {
  ...
  const done = active.results.filter(r => r.status === "done").length;
  const skipped = active.results.filter(r => r.status === "skipped").length;
  const total = active.results.length;
  const percent = Math.round((done / total) * 100);
  const avgMs = done ? Math.round(active.results.filter(r => r.status === "done").reduce((s, r) => s + r.durationMs, 0) / done) : 0;
...
function renderHistory() {
  ...
  if (!state.history.length) {
    el.innerHTML = `<div class="empty-state">Пока нет завершённых тренировок.</div>`;
    return;
  }

  el.innerHTML = state.history.slice(0, 12).map(item => `
...
function renderMotivation() {
  ...
  const doneCount = state.history.length + countCompletedRest();
  const plannedCount = state.plan.reduce((sum, w) => sum + w.days.filter(d => d.exercises.length).length, 0) || 1;
  ...
  const current = [...levels].reverse().find(l => state.points >= l.min) || levels[0];
  const next = levels.find(l => l.min > state.points);
  const xpPercent = next ? Math.round(((state.points - current.min) / (next.min - current.min)) * 100) : 100;
  ...
  const startWeight = state.measurements[0]?.weight || profile.weight;
  const target = profile.targetWeight;
  const diff = Math.abs(latestWeight - target);
  const wantsLoss = target < latestWeight;
  const weeklyRate = wantsLoss ? 0.6 : 0.35;
  const thirtyDayChange = weeklyRate * 4.285;
  const forecast = wantsLoss ? Math.max(target, latestWeight - thirtyDayChange) : Math.min(target, latestWeight + thirtyDayChange);
  const weeks = diff > 0 ? Math.ceil(diff / weeklyRate) : 0;

  $("forecastWeight").textContent = `${forecast.toFixed(1)} кг`;
  $("forecastText").textContent = wantsLoss
    ? `При темпе около -${weeklyRate} кг/нед.`
    : `При темпе около +${weeklyRate} кг/нед.`;
  $("weeksToGoal").textContent = weeks ? `${weeks} нед.` : "цель";
  $("startWeightLabel").textContent = `Старт ${startWeight} кг`;
  $("targetWeightLabel").textContent = `Цель ${target} кг`;

  const totalPath = Math.abs(startWeight - target) || 1;
  ...
  const checkpointLabel = nextCheckpoint();
  $("nextCheckpoint").textContent = checkpointLabel;
}

function countCompletedRest() {
  return state.plan.reduce((sum, w) => sum + w.days.filter(d => d.completed && !d.exercises.length).length, 0);
}

function calculateStreak() {
  if (!state.history.length) return 0;
  const dates = [...new Set(state.history.map(h => h.date.slice(0, 10)))];
  return Math.min(dates.length, state.history.length);
}

function getLatestWeight() {
  if (state.measurements.length) return Number(state.measurements[state.measurements.length - 1].weight);
...
function renderMeasurements() {
  ...
  body.innerHTML = state.measurements.map(m => `
    <tr>
      <td>${m.date}</td>
      <td>${m.weight || "—"}</td>
      <td>${m.waist || "—"}</td>
...
function exportJSON() {
  ...
  a.href = url;
  a.download = `fitmvp-progress-${todayISO()}.json`;
  a.click();
...
function bindEvents() {
  ...
  $("closeWorkoutBtn").addEventListener("click", () => {
    if (confirm("Закрыть тренировку? Текущий live-прогресс не будет засчитан как завершённый.")) {
      state.activeWorkout = null;
      save();
      closeWorkout();
    }
  ...
  $("resetBtn").addEventListener("click", () => {
    if (!confirm("Точно сбросить все данные FitMVP в этом браузере?")) return;
